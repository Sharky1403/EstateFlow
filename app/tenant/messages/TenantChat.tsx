...
export function TenantChat({ conversationId, landlordName }: { conversationId: string; landlordName: string }) {
  ...
  return (
    <div className="flex flex-col bg-white rounded-2xl border border-slate-200 shadow-card overflow-hidden" style={{ height: 'calc(100vh - 180px)' }}>
      {/* Header */}
      <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-800">{landlordName}</p>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center py-12">
            ...
            <p className="text-xs text-slate-400 mt-1">Send a message to your property manager</p>
          </div>
        )}
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
              <span className="text-xs text-slate-400 mb-1 px-1">
                ...
                {!isMe && m.profiles?.role === 'landlord' && ' · Manager'}
              </span>
